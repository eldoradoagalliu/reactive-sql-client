...
public class Movie {
    ...
    public static Uni<Movie> findById(PgPool client, Long id) {
        return client.preparedQuery("SELECT id, title FROM movies WHERE id = $1").execute(Tuple.of(id))
    ...
    public static Uni<Long> saveMovie(PgPool client, String title) {
        return client.preparedQuery("INSERT INTO movies (titile) VALUE ($1) RETURNING id")
